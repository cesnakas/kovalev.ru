...
(function() {
	...
	BX.Sale.OrderAjaxComponent = {
		...
		init: function(parameters)
		{
			...
			{
				this.totalInfoBlockNode = this.totalBlockNode.querySelector('.order__price');
		...
		showErrors: function(errors, scroll, showAll)
		{
			...
			{
				section = BX.findParent(errorNodes[k], {className: 'order__item'});
		...
		checkNotifications: function()
		{
			...
			if (informer)
			{
				...
				{
					sections = this.orderBlockNode.querySelectorAll('.order__item.bx-active');
		...
		{
			var sections = this.orderBlockNode.querySelectorAll('div.order__item.bx-active'),
				i, errorNode;
		...
		{
			var firstSection = this.orderBlockNode.querySelector('.order__item.bx-active');
		...
		locationsCompletion: function()
		{
			...
			if (this.activeSectionId !== this.regionBlockNode.id)
				this.editFadeRegionContent(this.regionBlockNode.querySelector('.order__item-content'));

			if (this.activeSectionId != this.propsBlockNode.id)
				this.editFadePropsContent(this.propsBlockNode.querySelector('.order__item-content'));
		},
		...
		clickNextAction: function(event)
		{
			...
			{
				titleNode = section.next.querySelector('.order__item-title-container');
				...
				allSections = this.orderBlockNode.querySelectorAll('.order__item.bx-active');
		...
		getNextSection: function(actionSection, skippedSection)
		{
			...
			var allSections = this.orderBlockNode.querySelectorAll('.order__item.bx-active'),
				nextSection, i;
		...
		{
			return BX.create('DIV', {props: {className: 'order__item-content' + (!!notFluid ? '' : ' container-fluid')}});
		},
		...
		changeVisibleContent: function()
		{
			var sections = this.orderBlockNode.querySelectorAll('.order__item.bx-active'),
				i, state;
			this.result.IS_AUTHORIZED = true;
		...
		changeVisibleSection: function(section, state)
		{
			...
			{
				content = section.querySelector('.order__item-content');
				if (content)
		...
		editOrder: function()
		{
			...
			var sections = this.orderBlockNode.querySelectorAll('.order__item.bx-active'), i;
			for (i in sections)
			{
				if (sections.hasOwnProperty(i))
				{
		...
		editAuthBlock: function()
		{
			...
			var authContent = this.authBlockNode.querySelector('.order__item-content'),
			...
			if (this.result.OK_MESSAGE && this.result.OK_MESSAGE.length)
			{
				...
				BX.prepend(okMessageNode, this.authBlockNode.querySelector('.order__item-content'));
			}
		},
		...
		toggleAuthForm: function(event)
		{
			...
			var target = event.target || event.srcElement,
				section = BX.findParent(target, {className: 'order__item'}),
				container = BX.findParent(target, {className: 'order__item-content'}),
			...
			section.appendChild(
				BX.create('DIV', {
					props: {className: 'order__item-title-container'},
					children: [
						BX.create('div', {
							attrs: {'data-entity': 'section-title'},
							props: {className: 'order__item-title'},
		...
		editFadeBasketBlock: function(activeNodeMode)
		{
			...
			if (this.initialized.basket)
			{
				this.basketHiddenBlockNode.appendChild(node.querySelector('.order__item-content'));
		...
		editCoupons: function(basketItemsNode)
		{
			var couponsList = this.getCouponsList(true),
				...
				couponsBlock = BX.create('DIV', {
					props: {className: 'bx-soa-coupon-block coupon-wrap__control'},
					children: [
						BX.create('DIV', {
							...
							children: [
								BX.create('INPUT', {
									props: {
										className: 'coupon-wrap__value bx-ios-fix',
										type: 'text',
                                ...
                                BX.create('a',{
                                    props: {
                                        className: 'coupon-wrap__btn',
                                        href: 'javascript:void(0);'
                                    },
                                    text: 'Применить',
                                    events: {
                                         click: BX.delegate(function(couponsBlock){
                                            var parCoup = BX.findParent(BX(event.target), {class: 'bx-soa-coupon-input'});
                                            //var childCoup = BX.findChild();
  											//var newCoupon = BX.getEventTarget(event);
                                            var newCoupon = BX.findChild(BX(parCoup), {class: 'coupon-wrap__value'})
											if (newCoupon && newCoupon.value)
											{
												this.sendRequest('enterCoupon', newCoupon.value);
												newCoupon.value = '';
			...
			basketItemsNode.appendChild(
				BX.create('DIV', {
					props: {className: 'bx-soa-coupon coupon-wrap'},
					children: [
						//couponsLabel,
						couponsBlock
					]
		...
		editActiveRegionBlock: function(activeNodeMode)
		{
			...
			{
				regionContent = node.querySelector('.order__item-content');
				if (!regionContent)
				{
                ...
                this.editPropsItems(regionContent, [1, 2, 3, 4]);
				//this.editPropsItems(regionContent, []);
		...
		{
			var regionContent = this.regionBlockNode.querySelector('.order__item-content'), newContent;
		...
		getDeliveryLocationInput: function(node)
		{
			...
			if (location && location[0] && location[0].output)
			{
				...
				insertedLoc = BX.create('DIV', {
					...
					props: {className: 'bx-soa-location-input-container'},
		...
		getZipLocationInput: function(node)
		{
			...
			if (zipProperty)
			{
				...
				propsItemNode = BX.create('DIV', {props: {className: "bx-soa-location-input-container"}});
		...
		{
			var paySystemContent = this.paySystemBlockNode.querySelector('.order__item-content'), newContent;
		...
		{
			var deliveryContent = this.deliveryBlockNode.querySelector('.order__item-content'), newContent;
		...
		activatePickUp: function(deliveryName)
		{
			...
			this.pickUpBlockNode.querySelector('[data-entity="section-title"]').innerHTML =
				'<span class="order__item-title-count"></span>' + BX.util.htmlspecialchars(deliveryName);
		...
		editActivePickUpBlock: function(activeNodeMode)
		{
			...
			{
				pickUpContent = node.querySelector('.order__item-content');
				if (!pickUpContent)
				{
		...
		{
			var pickUpContent = this.pickUpBlockNode.querySelector('.order__item-content'), newContent;
		...
		{
			var propsContent = this.propsBlockNode.querySelector('.order__item-content'), newContent;
		...
		insertLocationProperty: function(property, propsItemNode, disabled)
		{
			...
			if (property.getId() in this.locations)
			{
				...
				{
					propContainer = BX.create('DIV', {props: {className: 'input-control-box'}});
		...
		insertFileProperty: function(property, propsItemNode, disabled)
		{
			...
			else
			{
				...
				if (saved)
					propContainer = saved.querySelector('div.input-control-box');
		...
		alterProperty: function(settings, propContainer)
		{
			...
			for (i = 0; i < inputs.length; i++)
			{
				...
				BX.addClass(inputs[i], 'feedback-box__control bx-soa-customer-input bx-ios-fix');
			}
			...
			if (add.length)
			{
				...
				BX.bind(add, 'click', BX.delegate(function(e){
					var target = e.target || e.srcElement,
						targetContainer = BX.findParent(target, {tagName: 'div', className: 'input-control-box'}),
		...
		isValidPropertiesBlock: function(excludeLocation)
		{
			...
			for (i = 0; i < props.length; i++)
			{
				...
				propContainer = props[i].querySelector('.input-control-box');
				if (propContainer)
				{
					arProperty = this.validation.properties[id];
		...
		showValidationResult: function(inputs, errors)
		{
			...
			var input0 = BX.type.isElementNode(inputs[0]) ? inputs[0] : inputs[0][0],
				formGroup = BX.findParent(input0, {tagName: 'DIV', className: 'input-control-box'}),
				//label = formGroup.querySelector('input-control-box'),
			...
			for (i = 0; i < inputs.length; i++)
			{
				inputDiv = BX.findParent(inputs[i], {tagName: 'DIV', className: 'input-control-box'});
